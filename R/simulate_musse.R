...
# birth & death rates (=speciation & extintion rates) are evolving according to a discrete-state continuous-time Markov model, as defined by the transition rate matrix
...
simulate_musse = function(	Nstates,						# number of discrete possible states for the trait
							...
							root_state				= NULL,		# integer between 1 and Nstates, specifying the state of the root. If NULL, the root state is chosen randomly.
	...
	if(is.null(parameters$transition_matrix)) parameters$transition_matrix = matrix(0,nrow=Nstates, ncol=Nstates);
	...
	if(is.null(root_state)){
		root_state = sample.int(n=Nstates,size=1)
	}else if(!(root_state %in% (1:Nstates))){
		stop(sprintf("ERROR: Invalid root_state (%d): Must be an integer between 1 and %d",root_state,Nstates))
	}
	...
	results = generate_random_tree_Mk_rates_CPP(max_tips					= (if(is.null(max_tips)) -1 else max_tips),
												...
												root_state					= max(1,min(Nstates, root_state)) - 1,
	...
	return(list(success				= TRUE,
				...
				Ntransitions		= matrix(results$Ntransitions,ncol=Nstates,byrow=TRUE), # number of transition events between each pair of states
