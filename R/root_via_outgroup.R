# root (or re-root) a tree based on a specified outgroup tip or node
...
root_via_outgroup = function(tree, outgroup, update_indices=TRUE){ 
	...
	if(is.character(outgroup)){
		...
		if((outgroup<1) || (outgroup>Ntips+Nnodes)) stop(sprintf("ERROR: outgroup must be between 1 and %d (=Ntips+Nnodes), but instead is %d",Ntips+Nnodes,outgroup));
	}else{
		stop("ERROR: outgroup must be a character or integer")
	}
	new_root_node = match(outgroup, tree$edge[,2]);
	if(is.na(new_root_node)){ new_root_node = outgroup-Ntips; } # outgroup is actually the root node
