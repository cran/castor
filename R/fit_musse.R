...
fit_musse = function(	tree, 
						...
						tip_priors 				= NULL, 		# 2D numerical array of size Ntips x Nstates, listing the prior likelihood of each state at each tip. Can be provided alternatively to tip_pstates.
	...
	if(!is.null(tip_pstates)){
		...
		if(Nstates != ncol(tip_priors)) stop(sprintf("ERROR: Nstates (%d) differs from the number of columns in tip_priors (%d)",Nstates,ncol(tip_priors)))
		known_tips = which(rowSums(is.na(tip_priors))==0);
		if(length(known_tips)==0) stop("ERROR: All tip states are hidden");
		if(check_input){
			max_tip_prior = max(tip_priors[known_tips,])
	...
	if(verbose && (Nknown_tips<Ntips)) cat(sprintf("%sNote: %d out of %d tips have unknown state\n",verbose_prefix,Ntips-Nknown_tips,Ntips))
	...
	if(has_reveal_fractions){
		...
		reveal_fractions = rep(length(known_tips)/Ntips, times=NPstates);
	}
    ...
    if(root_prior[1]=="flat"){
		...
		root_prior_probabilities = numeric(0) # root prior will be set to the computed state-likelihoods at the root, and thus depend on the particular model parameters. 
	...
	if(any(is.na(first_guess$transition_matrix))){
		...
		fit = asr_mk_model(	subtree, 
							(if(is.null(tip_pstates)) tip_priors[extraction$new2old_tip,] else tip_pstates[extraction$new2old_tip]),
							Nstates 				= Nstates,
		...
		if(fit$success){
			...
			first_guess$transition_matrix[is.na(first_guess$transition_matrix)] = Nstates/((if(is.null(subtree$edge.length)) 1 else mean(subtree$edge.length))*log(Nknown_tips)/log(2.0))
		}
	...
	if(all(param_scales$birth_rates==0)){
		...
		param_scales$birth_rates[] = log(Ntips)/max(node_ages);
	}else{
	...
	if(verbose) cat(sprintf("%sPreparing objective function for fitting..\n",verbose_prefix))
	if(!is.null(tip_pstates)){
	...
	if(include_ancestral_likelihoods) ancestral_likelihoods = matrix(final$ancestral_likelihoods, ncol=Nstates, byrow=TRUE, dimnames=list(tree$tip.label,NULL)) # unflatten
	final$ML_root_state = final$ML_root_state + 1 # make index 1-based
	...
	return(list(success					= TRUE, 
				...
				ML_root_state			= final$ML_root_state, # maximum-likelihood estimate of the root state, based on the computed state-likelihoods
