...
asr_mk_model = function(tree, 
    ...
    if(is.null(transition_matrix)){
    	...
    	fit_single_trial = function(trial){
			...
			if(optimization_algorithm == "optim"){
				fit = stats::optim(initial_dense_rates, objective_function, method="L-BFGS-B", lower=rep(min_rate, Nrates))
				LL = -fit$value;
			}else{
				fit = stats::nlminb(initial_dense_rates, objective_function, lower=rep(0, Nrates), upper=rep(1e+50, Nrates))
				LL = -fit$objective;
			}
