...
# The transition matrix is estimated via maximum-likelihood fitting.
fit_mk = function(	trees, 										# either a single tree in phylo format, or a list of trees
					...
					root_prior 				= "auto",			# can be 'auto', 'flat', 'stationary', 'empirical' or a numeric vector of size Nstates, specifying the prior probabilities for the tree's root. Used to define the tree's likelihood, based on the root's marginal likelihoods
    ...
    if(root_prior[1]=="auto"){
    	...
    	if(is.null(temp_transition_matrix)){
    		# fit model with a flat root prior, then calculate stationary distribution of fitted Q
			...
			if(fit$success){
				...
				return(list(success=FALSE, error="Could not fit model with preliminary flat root prior, in order to get a stationary distribution"))
			}
    	}
    	root_prior_type = "custom"
    	root_prior_probabilities = get_stationary_distribution(temp_transition_matrix)
	...
	for(tr in 1:Ntrees){
		...
		focal_tip_pairs			= focal_tip_pairs[valids,]
	...
	objective_function = function(dense_rates){
		...
		for(tr in 1:Ntrees){
			focal_tree = trees[[tr]]
			results = ASR_with_fixed_rates_Markov_model_CPP(Ntips 							= length(focal_tree$tip.label),
	...
	if(is.null(loglikelihood) || any(is.na(fitted_rates)) || any(is.nan(fitted_rates))){
		return(list(success=FALSE, error="Fitting yielded NaN loglikelihood and/or rates"));
	}
